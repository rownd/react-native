...
export default function () {
     ...
     */
    const getChallengeIfPresent = useCallback(async (appId: string | undefined, challengeLookupValues: string[]): Promise<IChallenge | null>  => {

        if (!appId) {
            return null;
        }

        const challenges: Record<string, string[]> = JSON.parse(storage.getString('challenges') || '{}');
    ...
    const registerFingerprint = useCallback(async () => {
        ...
        try {
            ...
            const challengeEntry = await getChallengeIfPresent(state.app.id, [state.user?.data?.email, state.user?.data?.phone_number].filter(Boolean));

            const fingerprint = await getFingerprint();
            const payload: IFingerprint = await api.post('hub/auth/fingerprints', {
                headers: {
            ...
            const challengeLookupHashes = await computePossibleChallengeLookupValues(state.app.id, [state.user?.data?.email, state.user?.data?.phone_number].filter(Boolean));
            if (payload.challenge && challengeLookupHashes.length > 0) {
                const challenges = JSON.parse(storage.getString('challenges') || '{}');
                storage.set('challenges', JSON.stringify({
                    ...challenges,
    ...
    }, [api, state.app.id, state.auth.access_token, state.auth.is_verified_user, state.user?.data?.email, state.user?.data?.phone_number, getChallengeIfPresent, getFingerprint]);
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const existingChallenge = await getChallengeIfPresent(state.app.id, [state.user?.data?.email, state.user?.data?.phone_number].filter(Boolean));
    ...
    }, [getChallengeIfPresent, registerFingerprint, state.app.id, state.auth.access_token, state.user?.data?.email, state.user?.data?.phone_number]);
