...
export function DefaultContext({ config }: DefaultContextProps) {
    ...
    useEffect(() => {
        (async () => {
            try {
                const resp: any = await api.get('hub/app-config', {
                    headers: {
    ...
    const retrieveUserInfo = useCallback(() => {
        ...
        (async () => {
            const userInfo: UserInfoResp = await api.get(`me/applications/${state.app.id}/data`, {
                headers: {
            ...
            dispatch({
                ...
                payload: userInfo.redacted.reduce((acc: Record<string, any>, field: string) => {
    ...
    const retrieveAcls = useCallback(() => {
        ...
        (async () => {
            try {
                const appUserAcls: UserAclsResponse = await api.get(`me/applications/${state.app.id}/acls`, {
                    headers: {
                        Authorization: `Bearer ${state.auth.access_token}`
                    }
                }).json();
    ...
    useEffect(() => {
        ...
        if ((!state.user.data?.email && !state.user.data.phone_number) || isEqual(userDataRef.current, state.user.data)) {
            return;
        }
